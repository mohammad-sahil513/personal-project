...
async def _local_services(args: argparse.Namespace, run: ManualRun) -> None:
    ...
    run.write_stage_snapshot(stage_name="GENERATION", snapshot=updated, extra={"section_generation_results": updated.get("section_generation_results")})
    run.write_metadata(
        {
            "runner": "run_generation_manual.py",
            "mode": "local_services",
            "workflow_run_id": wf.workflow_run_id,
            "document_id": doc.document_id,
            "template_id": tpl.template_id,
...
async def _local_api(args: argparse.Namespace, run: ManualRun) -> None:
    ...
    with httpx.Client(timeout=httpx.Timeout(120.0, connect=30.0)) as client:
        ...
        latest = poll_until(
            ...
            is_done=lambda body: _phase_reached(str(body.get("current_phase", "")), "GENERATION") or str(body.get("status", "")) in {"FAILED", "COMPLETED"},
            poll_interval_sec=args.poll_interval_sec,
            max_wait_sec=args.max_wait_sec,
        )
        run.write_stage_snapshot(stage_name="GENERATION", snapshot=latest)
        run.write_metadata(
            {
                "runner": "run_generation_manual.py",
                "mode": "local_api",
                "base_url": base,
                "workflow_run_id": wf["workflow_run_id"],
                "note": "API mode captures generation phase snapshot from full workflow execution.",
            }
        )


def main() -> int:
    parser = argparse.ArgumentParser(description="Manual generation stage runner")
    add_common_args(parser, require_template=False)
    args = parser.parse_args()
    run = ManualRun.create(runner_name="run_generation_manual", output_root=args.output_root)
