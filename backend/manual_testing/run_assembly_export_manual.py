...
async def _local_services(args: argparse.Namespace, run: ManualRun) -> None:
    ...
    updated = wf_service.get_workflow(wf.workflow_run_id).to_dict()
    output_info = {}
    if updated.get("output_id"):
        out = OutputService().get_output(updated["output_id"]).to_dict()
        output_info = out
        artifact_path = out.get("artifact_path")
        if artifact_path and Path(artifact_path).exists():
            copied = run.run_dir / "final_output.docx"
            copied.write_bytes(Path(artifact_path).read_bytes())
    run.write_stage_snapshot(stage_name="ASSEMBLY_VALIDATION", snapshot=updated, extra={"output": output_info})
    run.write_stage_snapshot(stage_name="RENDER_EXPORT", snapshot=updated, extra={"output": output_info})
    run.write_metadata(
        {
            "runner": "run_assembly_export_manual.py",
            "mode": "local_services",
            "workflow_run_id": wf.workflow_run_id,
            "document_id": doc.document_id,
            "template_id": tpl.template_id,
            "output_id": updated.get("output_id"),
        }
    )
...
async def _local_api(args: argparse.Namespace, run: ManualRun) -> None:
    ...
    with httpx.Client(timeout=httpx.Timeout(120.0, connect=30.0)) as client:
        ...
        latest = poll_until(
            ...
            is_done=lambda body: _phase_reached(str(body.get("current_phase", "")), "RENDER_EXPORT")
            or str(body.get("status", "")) in {"FAILED", "COMPLETED"},
            poll_interval_sec=args.poll_interval_sec,
            max_wait_sec=args.max_wait_sec,
        )
        run.write_stage_snapshot(stage_name="ASSEMBLY_VALIDATION", snapshot=latest)
        run.write_stage_snapshot(stage_name="RENDER_EXPORT", snapshot=latest)
        ...
        run.write_metadata(
            {
                ...
                "note": "API mode captures assembly/export snapshots from full workflow execution.",
            }
        )


def main() -> int:
    parser = argparse.ArgumentParser(description="Manual assembly + export stage runner")
    add_common_args(parser, require_template=False)
    args = parser.parse_args()
    run = ManualRun.create(runner_name="run_assembly_export_manual", output_root=args.output_root)
